...
def extract_table(page):
    ...
    total_records = (
        " ".join(page.locator("text=Total Records")
        .inner_text()
        .split(":")[-1]
        .split())
    )

    total_amount = " ".join(page.locator(
    ...
    headers = [" ".join(h.split()) for h in header_row.locator("th").all_text_contents() if h.strip()]
    ...
    for i in range(visible):
        ...
        cleaned = tuple(" ".join(c.replace("\xa0", " ").split()) for c in cells)
...
def run_once(p):
    ...
    try:
        ...
        # --- 📥 CERTIFICATE DOWNLOADER LINK ---
        print("🔗 STARTING WHT CERTIFICATE DOWNLOADS...")
        try:
            download_wht_certificates(page, headers, data)
            print("🎉 ALL CERTIFICATES DOWNLOADED SUCCESSFULLY")
        except Exception as cert_err:
            print(f"⚠️ Certificate Download encountered an error: {cert_err}")
