...
def extract_table(page: Page):
    ...
    try:
        ...
        logger.info("Table found and attached to DOM")

        # Extract total records
        try:
            total_records_text = page.locator("text=Total Records").inner_text()
            total_records = total_records_text.split(":")[-1].strip()
            print(f"Total Records: {total_records}")
        except Exception as e:
            logger.warning(f"Could not extract Total Records: {e}")
            total_records = "N/A"
                ...
                for i in range(odd_count):
                    try:
                        ...
                        try:
                            ...
                            logger.warning(f"Retry failed for oddRow {i}: {retry_error}")

        if not data:
            raise Exception("No data scraped from table")

        logger.info(f"Table scraping completed successfully. Total rows scraped: {len(data)} (out of {total_row_count} total rows found)")
        return headers, data
