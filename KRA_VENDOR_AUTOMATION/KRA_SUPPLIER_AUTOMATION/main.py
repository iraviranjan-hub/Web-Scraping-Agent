...
from wht_certificate_downloader import download_wht_certificates
from json_saver import save_json, print_json_console, format_as_json
from sql_server_saver import save_to_sql_server, get_last_run_date, update_last_run_date, get_existing_paths
                ...
                while current_date <= end_date:
                    try:
                        ...
                        if not consult_successful:
                            ...
                            update_last_run_date(current_date, "No records found") 
                        ...
                        if data and len(data) > 0:
                            ...
                            logger.info("Reading existing certificate paths from database...")
                            existing_info = get_existing_paths()

                            # --- 📥 DOWNLOAD CERTIFICATES ---
                            logger.info("🔗 Downloading certificates for this batch...")
                            download_wht_certificates(page, headers, data, existing_info=existing_info)
                            ...
                            day_data_json = format_as_json(headers, data, start_index=total_records_processed + 1)
                            print_json_console(day_data_json)
                            all_scraped_data.extend(day_data_json)
                        ...
                        if rows_inserted > 0:
                            update_last_run_date(current_date, "processed")
                        
                        time.sleep(2)
                        
                    except KRAITaxSystemError as ke:
                        # This handles the specific KRA system error
                        error_msg = str(ke.error_details) if hasattr(ke, 'error_details') and ke.error_details else str(ke)
                        
                        # EXACT format requested by the USER
                        formatted_error = f"EXCEPTION DETECTED: KRA SYSTEM ERROR !!!\nDetails: {error_msg}\nMailing the error and terminating the process as requested..."
                        
                        logger.critical(f"KRA SYSTEM ERROR DETECTED: {error_msg}")
                        print(f"\n{formatted_error}")
                        
                        # Send the email
                        send_error_email(error_msg)
                        
                        # Terminate the process (by raising an exception and not retrying)
                        raise Exception(formatted_error)

                    except Exception as date_error:
                        logger.error(f"[ERROR] Error scraping date {current_date.strftime('%Y-%m-%d')}: {date_error}")
                        # If we have collected some data, let's return it instead of failing completely
                        if all_scraped_data:
                            logger.warning(f"Process interrupted, but returning {len(all_scraped_data)} records found so far.")
                            return all_scraped_data
                        raise 
                    
                    current_date += timedelta(days=1)
                
                # 10. Save JSON logs if needed
                if total_records_processed > 0:
                    logger.info(f"[OK] Scraping process completed. Total records: {total_records_processed}")
...
if __name__ == "__main__":
    ...
    try:
       ...
       # user_end_date = datetime.now() - timedelta(days=1)
        ...
        user_end_date = datetime.strptime(start_date_str, "%Y-%m-%d")
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            results = run_automation(user_pin, user_password, user_start_date, user_end_date)
            if results is not None:
                logger.info("[SUCCESS] AUTOMATION COMPLETED SUCCESSFULLY!")
                break
        except Exception as e:
            error_str = str(e)
            logger.error(f"[FAIL] Attempt {attempt} failed with error: {error_str}")
            
            # If it's a critical KRA system error, don't retry, just exit
            if "CRITICAL KRA SYSTEM ERROR" in error_str:
                logger.critical("[STOP] Critical KRA System Error detected. Process terminated.")
                sys.exit(1)
